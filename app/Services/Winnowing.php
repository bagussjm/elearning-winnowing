...
class Winnowing
{
    ...
    //input properties
    private $prime_number = null;
    private $n_gram_value = null;
    private $n_window_value = null;
    ...
    private function char2hash($string)
    {
        if (strlen($string) == 1) {
            ...
            for ($i = 0; $i < $length; $i++) {
                $result += ord(substr($string, $i, 1)) * pow($this->prime_number, $length - ($i + 1));
            }
            return $result;
        }
    }
    ...
    {
        $roll_hash = array();
        $pangkat = $this->n_gram_value;
        foreach ($ngram as $o => $ng) {

            // $nilai_hash = ord($ng[1])*(pow((int)$this->prime_number,($pangkat-)));
            // var_dump($nilai_hash);exit();
            // array_push($roll_hash, $nilai_hash);
